...
public class MemoryManagerTest {
    ...
    private SmithAIPlugin mockPlugin(int maxMessages) {
    ...
    public void testConversationCreatedForNpc(@TempDir File tempDir) {
        MemoryManager mm = new MemoryManager(mockPlugin(5), new File(tempDir, "memory"));
    ...
    public void testConversationPersistsAndReloads(@TempDir File tempDir) {
        UUID id = UUID.randomUUID();
        MemoryManager mm1 = new MemoryManager(mockPlugin(5), new File(tempDir, "memory"));
        Conversation conv1 = mm1.getConversation(id);
        conv1.addMessage("user", "hello");
        conv1.addMessage("assistant", "hi there");
        mm1.saveAll();

        MemoryManager mm2 = new MemoryManager(mockPlugin(5), new File(tempDir, "memory"));
    ...
    public void testConversationMaxMessages(@TempDir File tempDir) {
        MemoryManager mm = new MemoryManager(mockPlugin(3), new File(tempDir, "memory"));
        UUID id = UUID.randomUUID();
        Conversation conv = mm.getConversation(id);
